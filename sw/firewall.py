...
class firewall:
    ...
    def firewall_report(self, full=False):
        ...
        cnt_mac_dst_hit  = self.wb.read(self.ba+0x020)
        cnt_mac_src_hit  = self.wb.read(self.ba+0x024)
        cnt_ipv4_dst_hit = self.wb.read(self.ba+0x028)
        cnt_ipv4_src_hit = self.wb.read(self.ba+0x02C)
        cnt_ipv6_dst_hit = self.wb.read(self.ba+0x030)
        cnt_ipv6_src_hit = self.wb.read(self.ba+0x034)
        ...
        print("----------------------------------------")
        print("Number of MAC_DST_ADDR hits:  %d" % cnt_mac_dst_hit)
        print("Number of MAC_SRC_ADDR hits:  %d" % cnt_mac_src_hit)
        print("Number of IPV4_DST_ADDR hits: %d" % cnt_ipv4_dst_hit)
        print("Number of IPV4_SRC_ADDR hits: %d" % cnt_ipv4_src_hit)
        print("Number of IPV6_DST_ADDR hits: %d" % cnt_ipv6_dst_hit)
        print("Number of IPV6_SRC_ADDR hits: %d" % cnt_ipv6_src_hit)
        if full:
            print("----------------------------------------")
